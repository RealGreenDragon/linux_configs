...
```
sudo add-apt-repository -y main && sudo add-apt-repository -y universe && sudo add-apt-repository -y restricted && sudo add-apt-repository -y multiverse && sudo apt update -y
```
...
```
sudo apt update -y && sudo apt install -y build-essential dkms linux-headers-generic && sudo apt upgrade -y --with-new-pkgs && sudo apt autoremove -y && sudo apt clean -y
```
...
```
sudo apt update -y && sudo apt upgrade -y --with-new-pkgs && sudo apt install -y apt-transport-https build-essential ca-certificates curl dkms gawk git hunspell-en-us hunspell-it linux-tools-common linux-tools-generic lsb-release net-tools p7zip-full p7zip-rar perl python3 python3-doc python3-pip python3-venv sed software-properties-common unrar unzip vim wget zip && sudo apt autoremove -y && sudo apt clean -y
```
...
```
sudo add-apt-repository -y ppa:ubuntu-mozilla-security/ppa && sudo apt update -y
...
```
sudo dpkg --add-architecture i386 && wget -q -O - https://dl.winehq.org/wine-builds/winehq.key | sudo apt-key add - && sudo apt-add-repository 'deb https://dl.winehq.org/wine-builds/ubuntu/ $(lsb_release -cs) main' && sudo apt update -y && sudo apt install -y --install-recommends winehq-stable && winecfg
```
...
sudo apt update -y && sudo apt upgrade -y && sudo apt install -y ubuntu-restricted-extras ffmpeg
```

#### libdvdcss2
```
export DEBIAN_FRONTEND="noninteractive"
sudo apt update -y && sudo apt upgrade -y && sudo apt install -y libdvdnav4 "^libdvdread[0-9]" libdvd-pkg && sudo dpkg-reconfigure -f noninteractive libdvd-pkg
```
...
#### MKVToolNix + GUI
```
sudo wget -O /usr/share/keyrings/gpg-pub-moritzbunkus.gpg https://mkvtoolnix.download/gpg-pub-moritzbunkus.gpg && echo "deb [arch=amd64 signed-by=/usr/share/keyrings/gpg-pub-moritzbunkus.gpg] https://mkvtoolnix.download/ubuntu/ $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/mkvtoolnix.download.list && sudo apt update -y && sudo apt install -y mkvtoolnix mkvtoolnix-gui
```
...
```
curl -fsSL https://miktex.org/download/key | sudo tee /usr/share/keyrings/miktex-keyring.asc > /dev/null && echo "deb [signed-by=/usr/share/keyrings/miktex-keyring.asc] https://miktex.org/download/ubuntu $(lsb_release -cs) universe" | sudo tee /etc/apt/sources.list.d/miktex.list && sudo apt update -y && sudo apt install -y miktex && miktexsetup finish && initexmf --set-config-value [MPM]AutoInstall=1 && mpm --verbose --update && mpm --verbose --package-level=basic --upgrade && mpm --verbose --update
```

#### TeXstudio (IDE only, no TeXLive) -> Requirements: MiKTeX
```
sudo add-apt-repository -y ppa:sunderme/texstudio && sudo apt update -y && sudo apt -y --no-install-recommends install texstudio
```
...
sudo apt install -y wget curl pv jq
...
```
sudo apt install -y xterm python && sudo python -m pip install --upgrade ipaddress && sudo git clone --recursive https://github.com/KatharaFramework/Kathara.git /opt/kathara
...
```
sudo apt install -y wireshark && sudo usermod -aG wireshark $(whoami) && sudo reboot
```
...
#### qBittorrent
```
sudo add-apt-repository -y ppa:qbittorrent-team/qbittorrent-stable && sudo apt update -y && sudo apt install -y qbittorrent
```
